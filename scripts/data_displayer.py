...
def display_spikes(data, labels, title, reorder=False, normrows=True):
    plot_data = data
    plot_labels = labels
    if normrows:
    ...
    ax.imshow(plot_data, cmap='nipy_spectral', aspect='auto', interpolation='none', norm='symlog')
    ax.set_title(title)
    if len(plot_labels) > 0:
        ax.set_yticks(np.arange(len(plot_labels)), labels = plot_labels)
    plt.show()

def display_all(D, L, title):
    display_spikes(D, L, title, reorder=True)
