...
def get_sample_aggregates(
    ...
    initiator_labels, responder_labels = sumr.fill_labels(
        sample_groupings,
        initiator_labels=initiator_labels,
        responder_labels=responder_labels,
    )
    proactive_features, reactive_features, general_features = sumr.fill_features(
        sample_groupings,
        proactive_features=proactive_features,
        reactive_features=reactive_features,
        general_features=general_features,
    )
    sample_size, feature_type = sumr.find_sample_size_and_feature_type(
        sample_groupings,
        feature_name,
        proactive_features=proactive_features,
        reactive_features=reactive_features,
    ...
    for group_name in sample_groupings:
        ...
        M, ML0, ML1, ML2 = sumr.get_data_matrix(
            grouping,
            feature_name,
            collapse=collapse,
...
def test(
    ...
    dl.write_to_manifest_log(
        dl.STATISTICS_TYPE,
        f"Testing '{test_type}' (Collapse:{collapse}, Overwrite:{overwrite})",
    )
    for feature_name in feature_set:
        dl.write_to_manifest_log(
            dl.STATISTICS_TYPE, f"Doing statistics for '{feature_name}'"
        )
        feature_collapse = decide_collapse(collapse, feature_name)
        feature_results = get_sample_aggregates(
            res,
            feature_name,
        ...
        try:
            test_method(
                ...
                keys,
                savepath,
                sanitized_feature_name,
                sanitized_grouping_name,
                split_name,
                overwrite,
                collapse,
            )
            ...
            dl.write_to_manifest_log(
                dl.STATISTICS_TYPE,
                f"Doing statistics failed for feature '{feature_name}'.",
            )
            dl.log_stack(f"Failure in running experiments. ({e})")
...
def pairwise_test_proactive(
    ...
    if not overwrite and fullpath.exists():
        ...
        ttest = popa.compare_groupings(
            feature_results, popa.pairwise_comparison_proactive
        )
        ...
        dl.write_to_manifest_new_summary_file(
            dl.STATISTICS_TYPE,
            fullpath,
            f"{sanitized_feature_name}:{split_name}",
            f"{ttest_n}",
        )


def pairwise_test_reactive(
    feature_results,
    keys,
    savepath,
    sanitized_feature_name,
    ...
    if not overwrite and fullpath.exists():
        ...
        ttest = popa.compare_groupings(
            feature_results, popa.pairwise_comparison_reactive
        )
        ttest.to_csv(fullpath, sep="\t")
        ttest_n = len(ttest)
        dl.write_to_manifest_new_summary_file(
    ...
    try:
        ...
        dd.r(
            data1,
            f"Median {sanitized_feature_name} {collapse_string}'{keys[0].lower()}'",
            feature_results_1[nt.INIT_L_KEY],
            feature_results_1[nt.RESP_L_KEY],
            width=width,
            height=height,
            savefig=True,
            colorbar=True,
            subfolder=figure_folder,
            overwrite=overwrite,
        )
        dd.r(
            data2,
            f"Median {sanitized_feature_name} {collapse_string}'{keys[1].lower()}'",
            feature_results_2[nt.INIT_L_KEY],
            feature_results_2[nt.RESP_L_KEY],
            width=width,
            height=height,
            savefig=True,
            colorbar=True,
        ...
        dd.r(
            data2 - data1,
            f"Median {sanitized_feature_name} {collapse_string}difference between '{keys[1].lower()}' and '{keys[0].lower()}'",
            feature_results_2[nt.INIT_L_KEY],
            feature_results_2[nt.RESP_L_KEY],
            width=width,
            height=height,
        ...
        dl.log(f"Failed {sanitized_feature_name}. ({e})")


INTRAPGROUP_PROPERTIES = {
    "rating_names": ["rating_holistic", "rating_IC_score", "rating_turn_taking"]
}


def intragroup_test_proactive(
    feature_results,
    keys,
    savepath,
    sanitized_feature_name,
    ...
    if not overwrite and fullpath.exists():
        ...
        ttest = popa.compare_groupings(
            feature_results,
            popa.intragroup_comparison_proactive,
            INTRAPGROUP_PROPERTIES,
        )
        ttest.to_csv(fullpath, sep="\t")
        ttest_n = len(ttest)
...
def intragroup_test_reactive(
    ...
    if not overwrite and fullpath.exists():
        ...
        ttest = popa.compare_groupings(
            feature_results, popa.intragroup_comparison_reactive, INTRAPGROUP_PROPERTIES
        )
        ttest.to_csv(fullpath, sep="\t")
        ttest_n = len(ttest)
        dl.write_to_manifest_new_summary_file(
...
def analysis_precompute(res, collapse=True):
    ...
    performance_results = get_sample_aggregates(
        res,
        "performance_length",
        collapse=collapse,
        initiator_labels=initiator_labels,
    ...
    for rating in ratings:
        ratings_results[rating] = get_sample_aggregates(
            res,
            rating,
            collapse=collapse,
            initiator_labels=initiator_labels,
...
def run_statistics(
    ...
    dl.write_to_manifest_log(
        dl.STATISTICS_TYPE,
        f"Running statistics (Speakers: {len(speakers)}, Samples: {len(samples)}, Summaries: {len(list(summaries.keys()))})",
        level=0,
    )
    ...
    for grouping in groupings:
        ...
        dl.write_to_manifest_log(
            dl.STATISTICS_TYPE, f"Started statistics for grouping: '{grouping_name}'"
        )
        speaker_group = dfr.get_speaker_groups(speakers, grouping)
        group_count = len(speaker_group)
        dl.write_to_manifest_log(
            dl.STATISTICS_TYPE, f"Groups in '{grouping_name}': {group_count}"
        )
        ...
        (
            ...
            performance_results,
            ratings_results,
        ) = analysis_precompute(res, collapse=collapse)

        test_tasks = []
        if test_pairwise and (group_count == 2):
            if test_proactive_features:
                test_tasks.append((pairwise_test_proactive, nt.PROACTIVE_FEATURES_LIST))
            if test_reactive_features:
                test_tasks.append((pairwise_test_reactive, nt.REACTIVE_FEATURES_LIST))
        if test_intragroup and (group_count > 2):
            if test_proactive_features:
                test_tasks.append(
                    (intragroup_test_proactive, nt.PROACTIVE_FEATURES_LIST)
                )
            if test_reactive_features:
                test_tasks.append((intragroup_test_reactive, nt.REACTIVE_FEATURES_LIST))
        if test_nwise and (sumr.get_average_samples_count(sample_groupings) > 5):
            if test_proactive_features:
                test_tasks.append((nwise_test_proactive, nt.PROACTIVE_FEATURES_LIST))
            if test_reactive_features:
                test_tasks.append((nwise_test_reactive, nt.REACTIVE_FEATURES_LIST))
        ...
        for test_method, feature_set in test_tasks:
            try:
                ...
                dl.write_to_manifest_log(
                    dl.STATISTICS_TYPE,
                    f"Running statistics failed for test: '{test_method.__name__}'",
                )
                dl.log_stack(f"Failure in running experiments. ({e})")
        del res
