    ...
    for group_path in group_paths:
        if not isinstance(group_keywords, type(None)):
            ...
            for group_keyword in group_keywords:
                all_group_keywords_found = all_group_keywords_found and (
                    group_keyword in str(group_path)
                )
...
def get_test_collation(test_dir, group_keywords, random_choice=True):
    dfs = []
    csvs_to_collate, selected_group = get_csv_list(
        test_dir, group_keywords, random_choice=random_choice
    )
...
def get_collations(tasks=None, group_keywords=None, random_choice=True):
    ...
    for test_dir in test_dirs:
        test_collation = get_test_collation(
            test_dir, group_keywords=group_keywords, random_choice=random_choice
        )
...
def filter_normality(df, keys, norm_p, norm_val):
    ...
    for key in keys:
        ...
        for col in df.columns:
            if ("N-p" not in col) or (key not in col):
                continue
        ...
        for col in df.columns:
            if (("skew" not in col) and ("kurt" not in col)) or (key not in col):
                continue
...
def filter_nobs(df, keys, nob_ratio, hard_bottom=0):
    ...
    for key in keys:
        for col in df.columns:
            if ("nobs" not in col) or (key not in col):
                continue
            max_nobs = df[col].max()
...
def add_bonferroni_rejection(df, p_col, alpha=0.05):
    ...
    dl.log(
        f"Bonferroni correction rejected {ps_c.sum()}. Corrected alpha: {corrected_alpha}"
    )
...
def get_speaker_data(
    ...
    x_columns = []
    y_columns = []
    for row in enumerate(key_fields):
        row_key_fields = row[1]
        feat = row_key_fields[0]
        chan1 = row_key_fields[1]
        chan2 = row_key_fields[2]
        x_column = get_x_column(chan1, chan2, feat)
        x_columns.append(x_column)
        x_columns_to_fields[x_column] = {
            "feature": feat,
            "channel_1": chan1,
            "channel_2": chan2,
        }
        ...
        for sample_key in samples:
            ...
            for x_column in x_columns:
                ...
                if feature_type == nt.PROACTIVE:
                    val = sumr.get(
                        label_summaries, f"{l1}/self/{feat}", fallback=np.nan
                    )
...
def load_table(tasks, table_name, file_dir=None, criteria="bonferroni rejection (0.05)"):
    ...
    for i in range(len(test_name)):
        test_name[i] = test_name[i].capitalize()
    test_name = " ".join(test_name)
    df = pd.read_csv(
        iot.output_csvs_path() / file_dir / file_name, sep="\t", low_memory=False
    )
    df = filter_columns(df)
    df["channel_2"] = df["channel_2"].fillna("")
    selection_criteria = df[criteria]
    selected_df = df[selection_criteria]
    return df, selected_df, test_name
