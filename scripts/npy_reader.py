...
def get_summary_info_io(summary):
    (
        ...
        relational_total,
        valid_structure,
    ) = sumr.count_label_values(summary)
    ...
    if valid_structure:
        ...
        output_info = "invalid summary structue"
    return input_info, output_info


def write_summary(name, summary):
    npz_path = iot.DL_npzs_path() / nt.file_swap(name, "npz", all=False)
    npy_path = iot.summary_npys_path() / nt.file_swap(name, "npy", all=False)
    input_info = None
    output_info = None
    ...
    try:
        ...
        dl.log_stack(f"Failed to create summary info. ({e})")
...
def get_summary(name, D, L):
    ...
    D_binary = npzr.transform_to_binary(
        D_focused, discrete_channels, binary_channels, D_smooth=D_smooth
    )
    D_thresholded = npzr.transform_to_thresholded(
        D_focused, discrete_channels, binary_channels, D_smooth=D_smooth
    )
    D_continuous = npzr.transform_to_continuous(
        D_focused, discrete_channels, binary_channels, D_smooth=D_smooth
    )
    ...
    summary = dict()
    summary = {
        "label_summaries": ana.summarize_analyses(L, analyses, Ls),
        "general": {
...
def npy_list():
    npys = []
    for name in iot.list_dir_names(iot.summary_npys_path(), "npy"):
        npys.append(name)
    return npys
...
def load_summary_from_name(name):
    ...
    return load_summary(iot.summary_npys_path() / name)
