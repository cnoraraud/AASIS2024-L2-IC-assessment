...
class DRows:
    ...
    def value_to_string_array(self, value):
        ...
        if self.skip_bad:
            try:
                ...
                dl.log(f"Couldn't create string array from ({value}). {e}")
        for val in value:
            clean_val = None
    ...
    def text(self, skip_bad=False):
        ...
        rows = []
        for label, vals in zip(self.labels, self.values):
            if len(vals) == 0:
                continue
            justified_label = label.rjust(max_len, " ")
            justified_vals = self.vals_to_string(vals, max_val_len)
            row = f"{justified_label}:\t{justified_vals}"
            rows.append(row)
...
def describe_info_of_summaries(summaries):
    ...
    for summary_key in summaries:
        ...
        (
            ...
            fields,
            non_nan_values,
        ) = get_summary_info_full(summary["summary"])
...
def count_label_values(summary):
    ...
    try:
        ...
        if "name" in summary:
            name = summary["name"]
        dl.log(f"Summary for {name} found to have invalid structure. ({e})")
        valid_structure = False

    return (
        individual_values_valid,
        individual_values,
        relational_values_valid,
        relational_values,
        valid_structure,
    )
...
def get(obj, path, supress_escape=False, fallback=None):
    ...
    for i in range(len(path)):
        ...
        if isinstance(keyword, str) and len(keyword) > 0:
            if keyword[0] == G_BRC and (
                isinstance(current_obj, list) or isinstance(current_obj, dict)
            ):
                ...
                has_optional_get = get(
                    current_obj, [optional_keyword] + path[i + 1 :], fallback
                )
...
def get_channels(summaries):
    ...
    res_labels_self = get(
        summaries,
        f"{G_STANDARD_SUMMARY_ACCES}/{G_OPT}label_summaries/{G_OPTBRC}/self/label",
    )
    res_labels_others = get(
        summaries,
        f"{G_STANDARD_SUMMARY_ACCES}/{G_OPT}label_summaries/{G_OPTBRC}/others/{G_BRC}/other_label",
    )
    ...
    return {
        "L1": sorted(list(labels_1)),
        "L2": sorted(list(labels_2)),
        "L": sorted(list(labels_all)),
    }
...
def get_feature_sizes(summaries, feature_name):
    res_feature_self = get(
        summaries,
        f"{G_STANDARD_SUMMARY_ACCES}/{G_OPT}label_summaries/{G_OPTBRC}/self/{feature_name}",
    )
    res_feature_others = get(
        summaries,
        f"{G_STANDARD_SUMMARY_ACCES}/{G_OPT}label_summaries/{G_OPTBRC}/others/{G_OPT}/{feature_name}",
    )
    res_feature_general = get(
        summaries, f"{G_STANDARD_SUMMARY_ACCES}/general/{feature_name}"
    )
    feature_shapes = get_shapes(
        [res_feature_self, res_feature_others, res_feature_general]
    )
...
def get_feature_names(summaries):
    res_labels_proactive = get(
        summaries, f"{G_STANDARD_SUMMARY_ACCES}/{G_OPT}label_summaries/{G_OPTBRC}/self"
    )
    res_labels_reactive = get(
        summaries,
        f"{G_STANDARD_SUMMARY_ACCES}/{G_OPT}label_summaries/{G_OPTBRC}/others/{G_BRC}",
    )
    ...
    feature_names_all = get_keys(
        [res_labels_proactive, res_labels_reactive, res_labels_general]
    )
    return {
        "F1": feature_names_1,
        "F2": feature_names_2,
        "FG": feature_names_general,
        "F": feature_names_all,
    }
...
def apply_sample_groupings_to_summaries(summaries, sample_groupings):
    ...
    for grouping in sample_groupings:
        ...
        for sample_key in samples:
            ...
            for summary_key in summaries:
                ...
                if npy not in group_summaries:
                    group_summaries[npy] = {
                        "npy": copy.deepcopy(npy),
                        "speaker": copy.deepcopy(speaker),
                        "S": copy.deepcopy(S),
                        "summary": {"general": copy.deepcopy(summary["general"])},
                    }
                    for rating_key in ratings:
                        group_summaries[npy]["summary"]["general"][
                ...
                for label_summary_key in label_summaries:
                    ...
                    group_summaries[npy]["summary"][initiator_label] = copy.deepcopy(
                        label_summary
                    )
                ...
                group_summaries[npy]["summary"]["labels"] = npw.string_array(
                    new_label_list
                )
        ...
        if count_not_found > 0:
            dl.log(
                f'For grouping "{grouping}", {count_not_found} out of {count_total} speakers didn\'t have any recorded sessions.'
            )
        summary_groupings[grouping] = group_summaries
    if len(no_sessions_found) > 0:
        dl.log("The speakers without recorded sessions were:")
        for speaker in sorted(list(no_sessions_found)):
            dl.log(f"\t{speaker}")
    else:
        dl.log("No missing sessions.")
...
def get_samples(
    ...
    if isinstance(initiator_labels, type(None)):
        sample = get_first_non_nan(
            summary, f"summary/{G_OPT}label_summaries/general/{feature_name}"
        )
        if sanitize:
    ...
    for i, l1 in enumerate(initiator_labels):
        if not isinstance(responder_labels, type(None)):
            ...
            for j, l2 in enumerate(responder_labels):
                sample = get_first_non_nan(
                    summary,
                    f"summary/{G_OPT}label_summaries/{l1}/others/{l2}/{feature_name}",
                )
                if sanitize:
                    sample = sanitize_sample(sample, sanitize_value, sample_size)
                matrix[i, j] = sample
            ...
            sample = get_first_non_nan(
                summary, f"summary/{G_OPT}label_summaries/{l1}/self/{feature_name}"
            )
            if sanitize:
                sample = sanitize_sample(sample, sanitize_value, sample_size)
            matrix[i] = sample
...
def gather_session_samples(
    ...
    for summary_key in summaries:
        ...
        sample = get_samples(
            ...
            responder_labels=responder_labels,
            sanitize=sanitize,
        ...
        session_samples[npy] = {
            ...
            "speaker": speaker,
            "sample": sample,
            nt.INIT_L_KEY: initiator_labels,
            nt.RESP_L_KEY: responder_labels,
        }

    return session_samples


def seperate_speaker_from_session(
    speaker, sample, initiator_labels, responder_labels, collapse="none"
):
    other_speaker = nt.get_speaker_other(speaker)

    if not isinstance(initiator_labels, type(None)):
        new_initiator_labels = npw.string_array(initiator_labels)
        new_initiator_labels = npzr.replace_labels(
            new_initiator_labels, speaker, nt.SPEAKER_SELF
        )
        new_initiator_labels = npzr.replace_labels(
            new_initiator_labels, other_speaker, nt.SPEAKER_OTHER
        )
    else:
    ...
    if not isinstance(responder_labels, type(None)):
        ...
        new_responder_labels = npzr.replace_labels(
            new_responder_labels, speaker, nt.SPEAKER_SELF
        )
        new_responder_labels = npzr.replace_labels(
            new_responder_labels, other_speaker, nt.SPEAKER_OTHER
        )
    else:
...
def sessions_to_speakers(session_samples, collapse=False):
    ...
    for sample_key in session_samples:
        ...
        new_sample, new_initiator_labels, new_responder_labels = (
            seperate_speaker_from_session(
                S, sample, initiator_labels, responder_labels, collapse=collapse
            )
        )
        speaker_samples[sample_name] = {
            "npy": sample_name,
            "sample": new_sample,
            nt.INIT_L_KEY: new_initiator_labels,
            nt.RESP_L_KEY: new_responder_labels,
            "speaker": speaker,
        }
...
def map_samples_to_common_matrix(
    all_samples,
    all_initiator_labels=None,
    all_responder_labels=None,
    sanitize_value=np.nan,
    sample_size=(),
):
    # Finding Labels
    ...
    for i, sample_key in enumerate(all_samples):
        ...
        self_indecies = np.array(
            [
                np.where(all_initiator_labels == label)[0][0]
                for label in initiator_labels
            ]
        )
        other_indecies = np.array(
            [
                np.where(all_responder_labels == label)[0][0]
                for label in responder_labels
            ]
        )
...
def find_feature_type(self_features, others_features, general_features, feature_name):
    ...
    if (
        sum(
            [
                feature_name in self_features,
                feature_name in others_features,
                feature_name in general_features,
            ]
        )
        > 1
    ):
...
def fill_labels(sample_group, initiator_labels=None, responder_labels=None):
    if isinstance(initiator_labels, type(None)) or isinstance(
        initiator_labels, type(None)
    ):
...
def fill_features(
    sample_group, proactive_features=None, reactive_features=None, general_features=None
):
    if (
        isinstance(proactive_features, type(None))
        or isinstance(reactive_features, type(None))
        or isinstance(general_features, type(None))
    ):
...
def find_sample_size_and_feature_type(
    sample_group,
    feature_name,
    proactive_features=None,
    reactive_features=None,
    general_features=None,
    sample_size=None,
    feature_type=None,
):
    proactive_features, reactive_features, general_features = fill_features(
        sample_group,
    ...
    if isinstance(sample_size, type(None)):
        ...
        if len(all_sample_sizes) > 0:
            sample_size = get_feature_sizes(sample_group, feature_name).most_common(1)[
                0
            ][0]
    if isinstance(feature_type, type(None)):
        feature_type = find_feature_type(
            proactive_features, reactive_features, general_features, feature_name
        )
...
def get_data_matrix(
    ...
    initiator_labels, responder_labels = fill_labels(
        sample_group,
        initiator_labels=initiator_labels,
        responder_labels=responder_labels,
    )
    ...
    sample_size, feature_type = find_sample_size_and_feature_type(
        sample_group,
        feature_name,
        proactive_features=proactive_features,
        reactive_features=reactive_features,
    ...
    samples = gather_session_samples(
        sample_group,
        feature_name,
        initiator_labels=initiator_labels,
        responder_labels=responder_labels,
